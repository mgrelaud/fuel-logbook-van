...
import { ArrowDown, ArrowLeft, ArrowUp, Download, Plus } from "lucide-react";
...
export const Route = createFileRoute("/carburant")({
  ssr: false,
  head: () => ({
    meta: [
      { title: "Carburant — Boîte noire" },
      {
        name: "description",
        content: "Suivi de consommation du camping-car, en L/100 km.",
      },
    ],
  }),
  component: () => (
  ...
  return (
    <main className="safe-top safe-bottom min-h-screen px-4 pb-32">
      <header className="flex items-center gap-3 py-4">
        ...
        <h1 className="flex-1 text-2xl font-bold tracking-tight">Carburant</h1>
        <button
          onClick={() =>
            pleins.length ? downloadCsv(pleins) : toast.error("Aucun plein à exporter.")
          }
          className="rounded-full bg-secondary p-2.5 text-muted-foreground"
      ...
      <section className="card-surface p-6 text-center">
        ...
        <p className="num mt-2 text-6xl font-bold text-primary">
